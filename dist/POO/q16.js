...
export function questao16POO() {
    ...
    while (resposta.toLowerCase() !== 'n') {
        let tipo = Number(prompt('Insira o tipo de animal (1-mamifero/2-ave): '));
        let nome = String(prompt('Insira o nome do animal: '));
        let especie = String(prompt('Insira a espécie do animal: '));
        let idade = Number(prompt('Insira a idade do animal: '));
        if (tipo === 1) {
            let tipoAlimentacao = String(prompt('Insira o tipo de alimentação do mamífero: '));
            let newMamifero = new Mamifero(nome, especie, idade, tipoAlimentacao);
            animais.push(newMamifero);
        }
        else if (tipo === 2) {
            let migratoria = String(prompt('O animal é migratório? (s/n): ')).toLowerCase() === 's';
            let newAve = new Ave(nome, especie, idade, migratoria);
            animais.push(newAve);
        }
