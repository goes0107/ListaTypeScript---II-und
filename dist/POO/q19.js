...
export function questao19POO() {
    ...
    while (resposta.toLowerCase() !== 'n') {
        let tipoSensor = Number(prompt('Digite o tipo de sensor (1-temperatura/2-pressao): '));
        let codigo = String(prompt('Digite o código do sensor: '));
        let leitura = Number(prompt('Digite a última leitura do sensor: '));
        if (tipoSensor === 1) {
            let sensorTemp = new SensorTemperatura(codigo, leitura);
